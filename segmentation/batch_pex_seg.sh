...
if [ "${SLURM_ARRAY_TASK_ID}" -eq  "$task_chk" ]
    then python3 ~/code/pyto_seg_slurm/segmentation/batch_pex_seg.py -d $img_dir -ht $high_threshold -lt \
        $low_threshold  "${imgs[@]:$first_img}"
    else python3 ~/code/pyto_seg_slurm/segmentation/batch_pex_seg.py -d $img_dir -ht $high_threshold -lt \
